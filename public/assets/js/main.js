(function (window, document, $, undefined) {
    ...
    $('.account-details-form').submit(async function (event) {
        event.preventDefault();
        try {
            const response = await fetch($(this).attr('action'), {
                ...
                body: new URLSearchParams(new FormData(this).entries())
            });
    ...
    $('.single-select').each(function () {
        ...
        if (params.has(name) && $(this).find(`option[value="${params.get(name)}"]`).length) {
    ...
    $('.single-select').on('change', function () {
        ...
        if (value !== 'all') params.append(name, value);
        if (params.size === 0) window.location.href = `${window.location.origin}/shop`;
        else window.location.href = `${window.location.origin}/shop?${params.toString()}`;
    });
    ...
    $('.btn-prev, .btn-next').on('click', function () {
        ...
        window.location.href = `${window.location.origin}/shop?${params.toString()}`;
    });
    ...
    $('.add-to-cart').click(async function () {
        ...
        try {
            ...
            const quantity = parseInt($(this).parent().parent().find('.pro-qty').find('input').val()) || 1;
    ...
    $(document).on('change', '.cart-dropdown .quantity-input, .axil-cart-table .quantity-input', async function () {
        const quantity = parseInt($(this).val());
        $('.dec').attr('disabled', true);
        $('.inc').attr('disabled', true);
        try {
            const productId = parseInt($(this).data('product-id'));
            await fetch(window.location.origin + '/cart', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    $(document).on('click', '.delete-from-cart', async function () {
        $('.delete-from-cart').attr('disabled', true);
        try {
            const productId = parseInt($(this).data('product-id'));
            await fetch(window.location.origin + '/cart', {
                method: 'DELETE',
        ...
        $('.delete-from-cart').attr('disabled', false);
    });
...
async function syncCart() {
    try {
        const response = await fetch(window.location.origin + '/cart/data', {
            method: 'POST'
        });
        ...
        if ($('.cart-dropdown')) {
            if (data.products.length) {
                $('.cart-body ul').html(
                    data.products
                        .map(
                            (product) => `<li class="cart-item">
                                <div class="item-img">
                                    <a href="${window.location.origin}/products/${product.slug}"><img src="${product.thumb}" alt="${product.title}" /></a>
                                    <button class="close-btn delete-from-cart" data-product-id="${product.id}"><i class="fas fa-times"></i></button>
                                </div>
                                <div class="item-content">
                                    <h3 class="item-title"><a href="${window.location.origin}/products/${product.slug}">${product.title}</a></h3>
                                    <div class="item-price"><span class="currency-symbol">$</span>${product.price}</div>
                                    <div class="pro-qty item-quantity">
                                        <button class="dec qtybtn">-</button>
                                        <input type="number" class="quantity-input" data-product-id="${product.id}" value="${product.quantity}" />
                ...
                $('.cart-body ul').html('<p class="text-center">No products in cart</p>');
            }
            $('.cart-count').text(data.products.length);
            $('.subtotal-amount').text(`$${data.cart.totalPrice.toFixed(2)}`);
        }
        if ('.axil-cart-table') {
            if (data.products.length) {
                $('.axil-cart-table tbody').html(
                    data.products
                        .map(
                            (product) => `<tr>
                                <td class="product-remove">
                                    <a href="javascript:void(0)" class="remove-wishlist delete-from-cart" data-product-id="${
                                        product.id
                                    }">
                                ...
                                <td class="product-thumbnail">
                                    <a href="${window.location.origin}/products/${product.slug}">
                                        <img src="${product.thumb}" alt="${product.title}">
                                    </a>
                                </td>
                                <td class="product-title">
                                    <a href="${window.location.origin}/products/${product.slug}">${product.title}</a>
                                </td>
                                ...
                                <td class="product-quantity">
                                    <div class="pro-qty item-quantity">
                                        <button class="dec qtybtn">-</button>
                                        <input type="number" class="quantity-input" data-product-id="${
                                            product.id
                                ...
                                <td class="product-subtotal">
                                    <span class="currency-symbol">$</span>${(product.price * product.quantity).toFixed(
                                        2
                                    )}
                                </td>
                            </tr>`
                ...
                $('.axil-order-summery .order-subtotal td:eq(1)').html(`$${data.cart.totalPrice.toFixed(2)}`);
                $('.axil-order-summery .order-total td:eq(1)').html(`$${data.cart.totalPrice.toFixed(2)}`);
            } else {
                $('.axil-cart-table tbody').html(
                    '<tr><td colspan="6" class="text-center">No products in cart</td></tr>'
                );
                $('.axil-order-summery .order-subtotal td:eq(1)').html('$0.00');
                $('.axil-order-summery .order-total td:eq(1)').html('$0.00');
            }
        }
    } catch {
        if ($('.cart-dropdown')) {
